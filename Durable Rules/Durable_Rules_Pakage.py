...
# print
	...
	def output(c):
		print("Fact: {0}'s {1} is {2}".format(c.m.what_day, c.m.action, c.m.object))

	@when_all((+m.what_day) & (m.action == 'fatigue'))
	def output2(c):
		print("Fact: {0}, your {1} is {2}".format(c.m.what_day, c.m.action, c.m.object))

	@when_all((+m.object) & ( m.action !='class') & (m.action != 'fatigue') & (m.action != 'safe'))
	def output3(c):
		print("Fact: {0}, You'd better {1} {2}".format(c.m.what_day, c.m.action, c.m.object))
