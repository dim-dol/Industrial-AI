...
def display_flow(img,flow, stride=40):
    ...
    return img, norm_opt_flow

prev_frame = cv2.imread("D:/stitching/dog_a.jpg",cv2.IMREAD_COLOR)

prev_frame = cv2.cvtColor(prev_frame, cv2.COLOR_BGR2GRAY)
prev_frame = cv2.resize(prev_frame, (0,0), None, 0.5, 0.5)

frame = cv2.imread("D:/stitching/dog_b.jpg",cv2.IMREAD_COLOR)
...
img, norm_opt_flow = display_flow(frame, opt_flow)


flow_dualtvl1 = cv2.createOptFlow_DualTVL1()
opt_flow2 = flow_dualtvl1.calc(prev_frame, gray, None)
prev_frame = np.copy(gray)

img2, DualTVL1_opt_flow = display_flow(frame,opt_flow2)

cv2.imshow('optical flow', img)
cv2.imshow('optical flow magnitude', norm_opt_flow)
cv2.imshow('DualTVL1 optical flow', img2)
cv2.imshow('DualTVL1 optical flow magnitude', DualTVL1_opt_flow)
